...
<template>
  ...
  <div class="vtabs">
    <ul class="nav nav-tabs tabs-vertical" role="tablist">
      ...
      <li class="nav-item">
        <a class="nav-link" v-on:click="loadFormIfExist()" data-toggle="tab" href="#profile4" role="tab" aria-selected="false">
          <span class="hidden-sm-up">
            <i class="ti-user"></i>
          </span>
...
export default {
  ...
  methods: {
    ...
    getFormHtml(id) {
      let currentf = this.html.filter(form => {
        return form.id == id;
      });
    ...
    async getFormsHtml() {
      ...
      let ssfs = await fetch(
        "https://2020i.site/api/wsers/" +
          this.$root._route.params.id +
          "/signedforms?",
        {
          method: "GET",
      ...
      let fs = await fetch(
        "https://2020i.site/api/Forms?filter[where][FormType]=affiliate"
      )
        .then(res => res.json())
        .then(json => {
          return json;
  ...
  computed: {
    ...
    user: {
      get: function() {
        if (this.users.items) {
          this.$store.state.users.current = this.users.items.filter(u => {
            return u.id == this.afid;
          });
          console.log("TOP GETTER");
          console.log(
            this.users.items.filter(u => {
              return u.id == this.afid;
            })
          );
          return this.users.items.filter(u => {
            return u.id == this.afid;
          });
        }
        return [];
    ...
    afid: {
      get: function() {
        return this.$store.state.users.afid;
      },
      set: function(value) {
        this.afid = value;
      }

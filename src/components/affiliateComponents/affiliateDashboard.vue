        ...
        <div id="page-wrapper">
            ...
            <div class="">
             <div class="white-box">
                 <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Affiliate Application</h4>
                                  <affiliateApplication :afid="this.$store.state.account.user.user.id"></affiliateApplication>
                                <!-- Nav tabs -->
...
import { mapState, mapActions } from "vuex";
    export default {
        ...
        created(){
             // this.getUserForms(this.$store.state.account.user.user.id)
        },
        methods:{
           ...
           async getFormsHtml(){
               console.log("AFFFDASHHHHHHHHHHHHH")
                   let sfs = this.$store.state.account.user.user.signedforms
                        let ssfs = await fetch('https://2020i.site/api/wsers/'+this.$store.state.account.user.user.id+'/signedforms?',{
                            method:"GET",
        ...
        components: {
                affiliateformfields,practicefields,TopHeader,SideBar,forms,viewer,uploader,affiliateApplication
         }
    }
</script>
<style scoped>
